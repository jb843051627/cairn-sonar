...
func TestBug24_RouteCloneIsolation(t *testing.T) {
    original := model.Route{ID: "route-1", SurveyID: "survey-1", Stops: []model.RouteStop{{ChamberID: "chamber-1", Order: 1, DurationM: 10}}}
    clone := original.Clone(); clone.Stops[0].Order = 99
    if original.Stops[0].Order == 99 { t.Fatal("route clone aliases Stops") }
}
