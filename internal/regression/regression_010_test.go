...
func TestBug10_EchoSurveyLookupError(t *testing.T) {
    svc, _ := fixture(t)
    p := pulse("survey-missing")
    _, err := svc.AnalyzeEcho(context.Background(), p)
    if !errors.Is(err, service.ErrSurveyNotFound) { t.Fatalf("orphan echo lookup should fail: %v", err) }
}
