...
func TestBug18_ReviewRefreshesAnomalyCache(t *testing.T) {
    svc, _ := fixture(t)
    if err := svc.RaiseAnomaly(context.Background(), model.Anomaly{ID: "anomaly-1", SurveyID: "survey-1", EchoID: "echo-1", Kind: "void", Severity: 2, Evidence: []float64{1}}); err != nil { t.Fatal(err) }
    if _, err := svc.ListAnomalies(context.Background(), "survey-1"); err != nil { t.Fatal(err) }
    if err := svc.ReviewAnomaly(context.Background(), "anomaly-1", model.AnomalyAccepted, "reviewer-1", "confirmed"); err != nil { t.Fatal(err) }
    items, err := svc.ListAnomalies(context.Background(), "survey-1"); if err != nil { t.Fatal(err) }
    if items[0].State != model.AnomalyAccepted { t.Fatalf("cache still reports %s", items[0].State) }
}
