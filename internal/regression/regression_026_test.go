...
func TestBug26_ReviewCancellationAndReviewer(t *testing.T) {
    svc, _ := fixture(t)
    if err := svc.RaiseAnomaly(context.Background(), model.Anomaly{ID: "anomaly-1", SurveyID: "survey-1", EchoID: "echo-1", Kind: "void", Severity: 2}); err != nil { t.Fatal(err) }
     err := svc.ReviewAnomaly(context.Background(), "anomaly-1", model.AnomalyAccepted, "", "")
     if err == nil { t.Fatal("review without reviewer/comment was accepted") }
}
