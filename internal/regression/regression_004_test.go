...
func TestBug04_TransactionRollback(t *testing.T) {
    _, repo := fixture(t)
    err := repo.WithTx(context.Background(), failTx)
    if err == nil { t.Fatal("transaction callback should fail") }
    var count int
    if err := repo.DB().QueryRow("SELECT COUNT(*) FROM events WHERE kind='probe'").Scan(&count); err != nil { t.Fatal(err) }
    if count != 0 { t.Fatalf("failed transaction left %d events", count) }
}
