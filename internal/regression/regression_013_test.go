...
func TestBug13_SignalPacketMalformedLength(t *testing.T) {
    data := make([]byte, 17)
    data[9] = 20
    if _, err := codec.DecodeSignal(data); err == nil { t.Fatal("malformed signal length was accepted") }
    packets, err := codec.ReadSignalLines(strings.NewReader("\n")); if err != nil { t.Fatal(err) }
    if len(packets) != 0 { t.Fatalf("blank signal line produced %d packets", len(packets)) }
}
