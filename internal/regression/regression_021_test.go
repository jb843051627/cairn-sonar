...
func TestBug21_CloseSurveyCancellation(t *testing.T) {
    svc, _ := fixture(t)
    ctx, cancel := context.WithCancel(context.Background()); cancel()
    err := svc.CloseSurvey(ctx, "survey-1")
    ...
    if survey.Status != model.SurveyActive { t.Fatalf("cancelled close changed status to %s", survey.Status) }
}
