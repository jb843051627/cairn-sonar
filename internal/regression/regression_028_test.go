...
func TestBug28_ArchiveWriterCancellation(t *testing.T) {
     ctx, cancel := context.WithCancel(context.Background()); cancel()
    _, err := service.FileArchive{}.Write(ctx, model.Survey{ID: "survey-1"}, nil)
    if !errors.Is(err, context.Canceled) { t.Fatalf("want cancelled archive, got %v", err) }
}
