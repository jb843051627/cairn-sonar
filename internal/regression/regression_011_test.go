...
func TestBug11_BatchCancellationAndScope(t *testing.T) {
     svc, _ := fixture(t)
     ctx, cancel := context.WithCancel(context.Background()); cancel()
    count, err := svc.IngestBatch(ctx, "survey-1", nil)
    if !errors.Is(err, context.Canceled) || count != 0 { t.Fatalf("cancelled empty batch continued: count=%d err=%v", count, err) }
}
