...
func TestBug15_QueueCloseDoesNotCloseJobs(t *testing.T) {
    q := worker.NewQueue(2); q.Close()
    defer func() { if value := recover(); value != nil { t.Fatalf("submit after close panicked: %v", value) } }()
    q.Submit("survey-1")
}
