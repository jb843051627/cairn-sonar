...
func TestBug12_SignalWindowBounds(t *testing.T) {
    _, err := rules.DetectWindow(model.Pulse{Samples: []float64{1, 2}}, 0, 10, rules.DefaultAnalysisConfig())
    if !errors.Is(err, rules.ErrInvalidWindow) { t.Fatalf("want invalid window error, got %v", err) }
}
