...
func TestBug20_SchedulerRegistryRace(t *testing.T) {
    scheduler := worker.NewScheduler(context.Background()); defer scheduler.Stop()
    var wait sync.WaitGroup
    for i := 0; i < 8; i++ { wait.Add(1); go func(id int) { defer wait.Done(); for j := 0; j < 200; j++ { name := "survey-" + string(rune('a'+id)); scheduler.Add(worker.Schedule{Name: name, Interval: time.Hour, Run: func(context.Context) {}}); scheduler.Names(); scheduler.Remove(name) } }(i) }
    wait.Wait()
}
