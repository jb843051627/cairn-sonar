...
func TestBug23_MissingAnomalyIdentity(t *testing.T) {
    svc, _ := fixture(t)
    err := svc.ReviewAnomaly(context.Background(), "anomaly-missing", model.AnomalyAccepted, "reviewer-1", "not found")
    if !errors.Is(err, service.ErrAnomalyNotFound) { t.Fatalf("want anomaly identity, got %v", err) }
}
