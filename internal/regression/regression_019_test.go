...
func TestBug19_QueueCompleteRace(t *testing.T) {
    q := worker.NewQueue(256); defer q.Close()
    var wait sync.WaitGroup
    for i := 0; i < 12; i++ { wait.Add(1); go func() { defer wait.Done(); for j := 0; j < 300; j++ { q.Submit("survey-1"); q.Complete("survey-1") } }() }
    wait.Wait()
}
