...
func TestBug22_MissingArchiveIdentity(t *testing.T) {
    svc, _ := fixture(t)
    _, err := svc.GetArchive(context.Background(), "survey-never-archived")
    if !errors.Is(err, service.ErrArchiveFailed) { t.Fatalf("want archive missing identity, got %v", err) }
}
