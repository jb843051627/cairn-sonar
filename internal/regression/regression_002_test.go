...
func TestBug02_DefaultArchiveWriter(t *testing.T) {
    svc, _ := fixture(t)
    if err := svc.CloseSurvey(context.Background(), "survey-1"); err != nil { t.Fatal(err) }
    if err := svc.ArchiveSurvey(context.Background(), "survey-1"); err != nil { t.Fatalf("default archive should work: %v", err) }
}
