...
func TestBug17_CalibrationInstrumentIdentity(t *testing.T) {
    svc, _ := fixture(t)
    calibration := model.Calibration{ID: "cal-1", InstrumentID: "instrument-missing", SurveyID: "survey-1", ReferenceDB: 50, MeasuredDB: 50, Operator: "operator-1"}
    err := svc.CalibrateInstrument(context.Background(), calibration)
    if !errors.Is(err, service.ErrInstrumentNotFound) { t.Fatalf("want instrument identity, got %v", err) }
}
