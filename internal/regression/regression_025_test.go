...
func TestBug25_SignalExtractionCancellation(t *testing.T) {
    svc, _ := fixture(t)
    ctx, cancel := context.WithCancel(context.Background()); cancel()
    _, err := svc.Signals().ExtractAll(ctx, []model.Pulse{pulse("survey-1")})
    if !errors.Is(err, context.Canceled) { t.Fatalf("want cancelled extraction, got %v", err) }
}
