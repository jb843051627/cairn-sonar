...
func TestBug09_MissingInstrumentKeepsIdentity(t *testing.T) {
    svc, _ := fixture(t)
    p := pulse("survey-1")
    p.InstrumentID = "instrument-missing"
    err := svc.RecordPulse(context.Background(), p)
    if !errors.Is(err, service.ErrInstrumentNotFound) { t.Fatalf("want ErrInstrumentNotFound, got %v", err) }
}

func TestBug09_ClosedSurveyRejectsPulse(t *testing.T) {
    svc, _ := fixture(t)
    if err := svc.CloseSurvey(context.Background(), "survey-1"); err != nil { t.Fatal(err) }
    err := svc.RecordPulse(context.Background(), pulse("survey-1"))
    if !errors.Is(err, service.ErrSurveyNotAccepting) { t.Fatalf("want ErrSurveyNotAccepting, got %v", err) }
}

func TestBug09_StoreErrorChainPreserved(t *testing.T) {
    _, repo := fixture(t)
    ctx, cancel := context.WithCancel(context.Background())
    cancel()
    _, err := repo.GetSurvey(ctx, "survey-1")
