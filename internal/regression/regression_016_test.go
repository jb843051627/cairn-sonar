...
func TestBug16_BatchMapCancellation(t *testing.T) {
    ctx, cancel := context.WithCancel(context.Background()); cancel()
    called := 0
    results := worker.Map(ctx, []int{1, 2, 3, 4}, 1, func(context.Context, int) (int, error) { called++; return 1, nil })
    if called != 0 { t.Fatalf("cancelled batch invoked callback %d times", called) }
    if len(results) != 4 { t.Fatalf("want 4 result slots, got %d", len(results)) }
}
